		...
		const int grid_x, const int grid_y) 
{
	...
	if ( dirx > 0 ) {
		...
		block_id.z = disp_range;
		grid_id.y = ny; 

		d_iterate_direction_dirxpos <<< grid_id, block_id >>>(dirx,left_image,costs,accumulated_costs, nx, ny, disp_range);
		...
		d_iterate_direction_dirypos <<< grid_id, block_id >>>(diry,left_image,costs,accumulated_costs, nx, ny, disp_range);
		// iterate_direction_dirypos (diry,left_image,costs,accumulated_costs, nx, ny, disp_range);
	} 
	else if ( dirx < 0 ) {
		// RIGHT MOST EDGE
		...
		d_iterate_direction_dirxneg <<< grid_id, block_id >>> (dirx,left_image,costs,accumulated_costs, nx, ny, disp_range);
	} 
	else if ( diry < 0 ) {
		// BOTTOM MOST EDGE
		// Process every pixel along this edge only if dirx ==
		...
		d_iterate_direction_diryneg <<< grid_id, block_id >>>(diry,left_image,costs,accumulated_costs, nx, ny, disp_range);
	}
}
		...
		const int w, const int h, const int disp_range )
{
	...
	for(diry=-1; diry<2; diry++) {
		...
		d_iterate_direction( dirx,diry, d_leftIm, d_costs, d_dir_accumulated_costs, nx, ny, disp_range, block_x, block_y, block_z, grid_x, grid_y);
		// d_iterate_direction( dirx,diry, d_leftIm, costs, d_costs, dir_accumulated_costs, d_dir_accumulated_costs, nx, ny, disp_range, block_x, block_y, block_z, grid_x, grid_y);
